...
def main():
    ...
    train_dataset = Dataset(opt, training=True, eval_split = opt.train_split, trucksc_version=opt.trucksc_version)
    ...
    test_dataset = Dataset(opt, training=False, eval_split = opt.test_split, trucksc_version=opt.trucksc_version)
    ...
    for epoch in range(start_epoch, opt.num_epochs):
        ...
        # checkpoint
        # -----------------------
        if epoch % 100 == 0 or epoch == opt.num_epochs - 1:
        ...
        with torch.no_grad():
            ...
            if accelerator.is_main_process:
                ...
                accelerator.print(
                    f"[eval] epoch: {epoch} "
                    f"loss: {eval_loss.item():.6f} "
                    f"cd: {eval_cd.item():.6f} "
                    f"rcs: {eval_rcs.item():.6f} "
                    f"vrel: {eval_vrel.item():.6f} "
                )
