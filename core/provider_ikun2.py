...
class ObjaverseDataset2(Dataset):
    def __init__(self, opt: Options, training=True):
        ...
        # 只取前 1000 个
        limit = getattr(self.opt, 'dataset_limit', 220)  # 默认 1000，可改
        if limit is not None:
            self.items = self.items[:int(limit)]
        print(f"[ObjaverseDataset2] using first {len(self.items)} ids")
    ...
    def __getitem__(self, idx):
        ...
        if self.plucker_ray:
            ...
            final_input = {'images': images_input}
        else:
            images_input = TF.normalize(images_input, IMAGENET_DEFAULT_MEAN, IMAGENET_DEFAULT_STD)
            final_input = {'images': images_input}
        ...
        # ===== RADAR (随机点云) =====
        # 默认配置；也可以在 self.opt 里加同名字段，用 getattr 读，便于命令行覆盖
        xy_range   = getattr(self.opt, 'radar_xy_range', 50.0)   # x,y ∈ [-xy_range, xy_range]
        z_value    = getattr(self.opt, 'radar_z_value', 1.0)     # z 固定
        min_pts    = getattr(self.opt, 'radar_min_pts', 500)     # 随机点数下限
        max_pts    = getattr(self.opt, 'radar_max_pts', 2500)    # 随机点数上限
        fixed_pts  = getattr(self.opt, 'radar_num_points', 1300) # 固定点数（可选）
        rcs_low    = getattr(self.opt, 'radar_rcs_low', -50.0)     # rcs 下界（线性）
        rcs_high   = getattr(self.opt, 'radar_rcs_high', 50.0)    # rcs 上界（线性）
        vmax       = getattr(self.opt, 'radar_vmax', 50.0)       # 速度范围：[-vmax, vmax]

        # 1) 确定点数
        if fixed_pts is not None:
            P = int(fixed_pts)
        else:
            P = random.randint(int(min_pts), int(max_pts))

        # 2) 位置：x,y∈[-xy_range, xy_range]；z 固定为 z_value
        xy = (torch.rand(P, 2, dtype=torch.float32) * 2 - 1) * float(xy_range)  # [P,2]
        z  = torch.full((P, 1), float(z_value), dtype=torch.float32)            # [P,1]
        points = torch.cat([xy, z], dim=-1)                                     # [P,3]

        # 3) rcs：线性标度（如需 dBsm 可自己转）
        rcs = torch.rand(P, 1, dtype=torch.float32) * (rcs_high - rcs_low) + rcs_low  # [P,1]

        # 4) 相对速度分量 vrel_x/y/z ∈ [-vmax, vmax]
        vrel = (torch.rand(P, 3, dtype=torch.float32) * 2 - 1) * float(vmax)    # [P,3]

        # 5) 写入 results
        results['radar_gt'] = {
            'points': points,   # [P,3] -> pcd.pc_data['x/y/z']
            'rcs':    rcs,      # [P,1] -> pcd.pc_data['rcs']
            'vrel':   vrel,     # [P,3] -> pcd.pc_data['vrel_x/y/z']
        }
...
if __name__ == "__main__":
    ...
    train_dataset = ObjaverseDataset2(opt=opt,training=True)
